...
public class HomeController {
	...
	public String log(@PathVariable(value = "id") Long id, @PathVariable(value = "idPage") int idPage, Model model,
			HttpSession session) {
		Post post = new Post(idPage);
		post.setId(id);
		session.setAttribute("post", post);
	...
	private void addPost(Model model, Auteur a, Post post) {
		...
		{
			if (a.getRole() == AuteurRole.ADMIN || a.getRole() == AuteurRole.MODERATOR) {
				metier.addSousCategorie(new SousCategorie(post.getContenu(), metier.getCategorie(id), true));
			} else {
				Message bean = new Message("Votre suggestion à bien été enregistrée. Merci.");
				model.addAttribute("erreurBean", bean);
				metier.addSousCategorie(new SousCategorie(post.getContenu(), metier.getCategorie(id), false));
			}
			model.addAttribute("sousCategories", metier.getSousCategoriesByCategorie(id));
		} else if (idPage == 3)// add sujet
		{
			if (a.getRole() == AuteurRole.ADMIN || a.getRole() == AuteurRole.MODERATOR) {
				Sujet sj = new Sujet(post.getIntitule(), post.getContenu(), true, (post.getNotifCreateur() != null), a,
						metier.getSousCategorie(id));

				sj.setPicture(post.getPicture());
	...
	protected String update(@Valid Auteur a, BindingResult bindingResult,
			@RequestParam(name = "Annuler", required = false) String annuler, Model model, MultipartFile file,
			HttpSession session) {
		Post post = (Post) session.getAttribute("post");
		int idPage = post.getIdPage();
		Long id = post.getId();
	...
	@RequestMapping(value = "/photoSujet", produces = MediaType.IMAGE_JPEG_VALUE)
	@ResponseBody
	public byte[] getPicture(Long id) throws IOException {
		Sujet a = metier.getSujet(id);
		if (a.getPicture() == null)
			return new byte[0];
		else
			return IOUtils.toByteArray(new ByteArrayInputStream(a.getPicture()));
	}
